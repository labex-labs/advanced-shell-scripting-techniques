# Fortgeschrittene Shell-Skripting-Techniken

**Sprachen:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/de/courses/advanced-shell-scripting-techniques">
    <img src="https://course-cover.labex.io/advanced-shell-scripting-techniques.png?lang=de" alt="Fortgeschrittene Shell-Skripting-Techniken">
  </a>
</p>

Lernen Sie fortgeschrittene Shell-Skripting-Techniken. Dieses Modul behandelt Grundlagen von Shell-Funktionen, Variablen-Scope und Rückgabestatus, Fehlerbehandlung mit `trap`, Techniken zur String-Manipulation und die Verarbeitung von Arrays. Sie werden diese wesentlichen Linux-Fähigkeiten durch praktische Übungen (Labs) und reale Herausforderungen meistern.

[Kurs auf LabEx starten](https://labex.io/de/courses/advanced-shell-scripting-techniques)

## Übungen

|   Index | Name                                           | Schwierigkeit   | Übung                                                                                                                                                 |
|---------|------------------------------------------------|-----------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
|       1 | Grundlagen von Shell-Funktionen                | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/shell-functions-basics-636486?course=advanced-shell-scripting-techniques'>Lab starten</a>           |
|       2 | Variablengültigkeitsbereich und Rückgabestatus | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/variable-scope-and-return-status-636488?course=advanced-shell-scripting-techniques'>Lab starten</a> |
|       3 | Fehlerbehandlung mit Trap                      | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/error-handling-with-trap-636484?course=advanced-shell-scripting-techniques'>Lab starten</a>         |
|       4 | Techniken zur String-Manipulation              | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/string-manipulation-techniques-636487?course=advanced-shell-scripting-techniques'>Lab starten</a>   |
|       5 | Arrays verarbeiten                             | Anfänger        | <a target='_blank' href='https://labex.io/de/labs/processing-arrays-636485?course=advanced-shell-scripting-techniques'>Lab starten</a>                |
|       6 | Log-Rotator-Skript                             | Fortgeschritten | <a target='_blank' href='https://labex.io/de/labs/log-rotator-script-636711?course=advanced-shell-scripting-techniques'>Challenge starten</a>         |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

